...
TEMPERATURE = 0.1  # 降低温度以保证结构化输出的稳定性
MAX_TOKENS = 32000
OUTPUT_DIR = r"d:\DM\LangChain\联络名单-Agent"
...
"""
    ...
    def handle_merged_cells(self, sheet, header_row_idx: int):
        ...
        for merged_range in merged_ranges:
            ...
            # 填充值
            is_header_area = (min_row - 1) == header_row_idx # openpyxl row从1开始，header_row_idx从0开始
            
            for row in range(min_row, max_row + 1):
                for col in range(min_col, max_col + 1):
                    cell = sheet.cell(row=row, column=col)
                    if is_header_area:
                        # 表头区域：如果是同一个合并单元格拆分出来的，添加后缀区分
                        # 这里简单处理：先填原值，后续统一去重处理
                        # 实际上，合并的表头通常是多级表头或跨列分类，这里按需求"添加后缀"
                        # 更好的策略是：先填值，最后读取整行表头时再做唯一化
    ...
    def unique_headers(self, headers: List[str]) -> List[str]:
        ...
        new_headers = []
        for h in headers:
            h_str = str(h) if h is not None else "Unknown"
            if h_str in seen:
                seen[h_str] += 1
...
4. 只返回JSON代码，不要包含Markdown格式（如 ```json ... ```）。
"""
        try:
            response = self.llm.invoke([HumanMessage(content=prompt)])
            content = response.content.strip()
    ...
    def process_sheet(self, file_path: Path, wb, sheet_name: str):
        ...
        # 3. 获取处理后的表头
        # openpyxl 行号从1开始，所以是 header_row_idx + 1
        header_row_values = []
        for cell in sheet[header_row_idx + 1]:
            header_row_values.append(cell.value)
        
        unique_headers_list = self.unique_headers(header_row_values)
        logger.info(f"处理后的表头: {unique_headers_list}")
        ...
        if output_file.exists():
            try:
                with open(output_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"发现已有进度，已处理 {processed_count} 条记录，将跳过。")
            except json.JSONDecodeError:
        ...
        # 收集所有数据行（为了支持大文件，使用iter_rows）
        # 注意：openpyxl 的 iter_rows 是生成器，内存友好
        ...
        if processed_count == 0:
            ...
            # 如果是追加模式，需要处理JSON结尾
            # 这里为了简化，我们假设已经是一个完整的list，我们读取出来再append可能内存不够
            # 为了大文件原子写入，更好的方式是 Line-delimited JSON (JSONL) 或者 追加到 list
            # 题目要求格式是 [ {...}, ... ]
            # 我们采取追加写入 ",\n {...}" 的方式，需要去除文件末尾的 "]"
        ...
        for row in row_iterator:
            ...
            # 断点跳过逻辑
            if skip_counter < processed_count:
                skip_counter += 1
                continue
            ...
            if len(current_batch) >= 10:
                json_chunk = self.process_chunk_with_llm(unique_headers_list, current_batch)
                self._append_json_to_file(output_file, json_chunk, is_first_batch=(processed_count == 0 and total_saved == 0))
                total_saved += len(json_chunk)
                current_batch = []
                logger.info(f"工作表 {sheet_name}: 已保存 {total_saved} 条数据")
        ...
        logger.info(f"工作表 {sheet_name} 处理完成，共保存 {total_saved} 条数据。")
    ...
    def process_file(self, file_path_str: str):
        ...
        try:
            # 使用 openpyxl 加载，data_only=True 获取计算后的值
            # 对于大文件，如果不需要写回，read_only=True 更快，但 merged_cells 在 read_only 模式下可能有兼容性问题
            # 这里题目要求"拆分合并单元格"，必须访问 merged_cells。
            # 为了 100MB+ 支持，我们尽量优化，但 openpyxl 加载整个结构是必须的。
            # 只有在非常大的时候才会 OOM。如果真的非常大，需要用 SAX 模式解析 merge_cells。
            # 鉴于通常 100MB xlsx 还可以接受，我们用标准加载。
